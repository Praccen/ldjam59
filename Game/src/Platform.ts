...
export class Platform {
    ...
    placeBlockFromRayCast(type: BlockType, camera: Camera, player: Player) {
        ...
        let hit = this.physicsScene.doRayCast(ray, true, [player.physicsObject], 100.0);
        if (hit.object == undefined) {
            return;
        }

        if (!this.physicsObjectIdToAttachedBlocksKey.has(hit.object.physicsObjectId)) {
            return;
        }

        let hitPosition = vec3.scaleAndAdd(vec3.create(), camera.getPosition(), camera.getDir(), hit.distance);
        let hitPositionInLocalCoords = vec3.transformMat4(vec3.create(), hitPosition, mat4.invert(mat4.create(), this.baseBlock.physicsObject.transform.matrix));

        let diff = vec3.sub(vec3.create(), hitPositionInLocalCoords, hit.object.physicsObjectId == this.baseBlock.physicsObject.physicsObjectId ? vec3.create() : hit.object.transform.position);
        let offsetComparedToHitObject = vec3.create();
        if (Math.abs(diff[0]) > Math.abs(diff[1]) && Math.abs(diff[0]) > Math.abs(diff[2])) {
            vec3.normalize(offsetComparedToHitObject, vec3.set(diff, diff[0], 0.0, 0.0));
        }
        else if (Math.abs(diff[1]) > Math.abs(diff[0]) && Math.abs(diff[1]) > Math.abs(diff[2])) {
            vec3.normalize(offsetComparedToHitObject, vec3.set(diff, 0.0, diff[1], 0.0));
        }
        else {
            vec3.normalize(offsetComparedToHitObject, vec3.set(diff, 0.0, 0.0, diff[2]));
        }

        this.addBlock(vec3.add(vec3.create(), hit.object.physicsObjectId == this.baseBlock.physicsObject.physicsObjectId ? vec3.create() : hit.object.transform.position, offsetComparedToHitObject), type);
    }
    ...
    removeBlockFromRayCast(camera: Camera, player: Player) {
        ...
        this.removeBlock(this.physicsObjectIdToAttachedBlocksKey.get(hit.object.physicsObjectId)!);
    }
